...
describe('Test API endpoint to comment on articles', () => {
  ...
  it('it should create a comment', (done) => {
    chai.request(app)
      ...
      .end((err, res) => {
        ...
        expect(res.body.comment.user.username).to.equal('steven');
        done();
      });
  });
  ...
  it('it should get a comment', (done) => {
    chai.request(app)
      .get('/api/articles/how-to-train-your-dragon-2/comments/1')
      .set('authorization', userToken)
      .end((err, res) => {
        expect(res).to.have.status(200);
        expect(res.body.comment.body).to.equal('This article is cool');
        expect(res.body.comment.user.username).to.equal('steven');
        done();
      });
  });
  ...
  it('it should return error if article does not exixt', (done) => {
    chai.request(app)
      .get('/api/articles/how-to-train-me/comments/7')
      .set('authorization', userToken)
      .end((err, res) => {
        expect(res).to.have.status(404);
        expect(res.body.message).to.equal('Article does not exist');
  ...
  it('it should return error if comment does not exixt', (done) => {
    chai.request(app)
      .get('/api/articles/how-to-train-your-dragon/comments/77')
      .set('authorization', userToken)
      .end((err, res) => {
        expect(res).to.have.status(404);
        expect(res.body.message).to.equal('Comment does not exist');
  ...
  it('it should not get a comment', (done) => {
    chai.request(app)
      .get('/api/articles/how-to-train-your-dragon/comments/55')
      .set('authorization', userToken)
      .end((err, res) => {
        expect(res).to.have.status(404);
        expect(res.body.message).to.equal('Comment does not exist');
